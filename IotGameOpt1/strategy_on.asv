...
bb=floor(4*rand(80,7));%重新部署100个普通节点
subnum.ON{1,om_m}=[aa;bb];%将新节点加入原区域
ff=sum(sum(bb),2);%新增加的传感器的数量
dd=sum(bb.*repmat(sensorvalue,80,1),2);%新增加的个节点的权重值
ee=sum(sum(dd),2);%新增加的权重值
cc=subwight.ON{1,om_m};%原区域各节点权重值
subwight.ON{1,om_m}=[cc;dd];%将新节点权重值加入原区域
subair.CH{1,om_m}=[subair.CH{1,om_m};bb.*repmat(sensorvalue,80,1)];%将新节点的各传感器权重值加入原区域
