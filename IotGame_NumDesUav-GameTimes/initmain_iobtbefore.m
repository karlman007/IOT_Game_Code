...
GS_weight=subairnum*subwight.SN{1,airnum};GS_num=subairnum*subnum.SN{1,airnum};
...
for chknow=0.08
for snknow=0.2 
P.on=onknow;P.ch=chknow;P.sn=snknow;P.gs=0.99;%攻击ON、CH、SN、GS节点，无人机被击毁概率
...
hive_payoff_hivebefore=zeros(4);%用于记录防御方为leader的支付函数
iobt_payoff_hivebefore=zeros(4);%用于记录防御方为leader的支付函数
hive_payoff_iobtbefore=zeros(4);%用于记录防御方为leader的支付函数
iobt_payoff_iobtbefore=zeros(4);%用于记录防御方为leader的支付函数
...
uav_gain/GS_weight-uav_consume/uav_m%-hive.lost/(hive_init.value)+(hive_after.xiufu)/(hive_init.value)+sensor_lost/10000;
%iobt支付函数
iobt_payoff(strategy_hive_define,strategy_iot)=...
uav_consume/uav_m-uav_gain/GS_weight%+hive.lost/(hive_init.value)+battle_ability/GS_weight+(sensor_add-sensor_lost)/10000-buildcost;
%%*********************************************************************%
%%
%蜂巢方先采取策略
...
strategy_hive_chose(strategy_hive,uav_m,P,subnum_init,subwight_init,neednum_init,hive_init,air_chose,subair_init,time_druing);
[buildcost,battle_ability,hive,sensor_add,~,~,~] =...
    strategy_iot_chose(strategy_iot,subnum,subwight,subair,subairnum,hive,time_druing);
%蜂巢支付函数
hive_payoff_hivebefore(strategy_hive_define,strategy_iot)=...
uav_gain/GS_weight-uav_consume/uav_m-hive.lost/(hive_init.value)+(hive.xiufu)/(hive_init.value)+sensor_lost/10000;
%iobt支付函数
iobt_payoff_hivebefore(strategy_hive_define,strategy_iot)=...
uav_consume/uav_m-uav_gain/GS_weight+hive.lost/(hive_init.value)+battle_ability/GS_weight+(sensor_add-sensor_lost)/10000-buildcost;
%%
%*********************************************************************%
%iobt方先采取策略
[buildcost,battle_ability,hive,sensor_add,subnum,subwight,subair] =...
    strategy_iot_chose(strategy_iot,subnum_init,subwight_init,subair_init,subairnum,hive_init,time_druing);
[uav_consume,uav_gain,sensor_lost,hive,~,~,~] =...
strategy_hive_chose(strategy_hive,uav_m,P,subnum,subwight,neednum_init,hive,air_chose,subair,time_druing);
%蜂巢支付函数
hive_payoff_iobtbefore(strategy_hive_define,strategy_iot)=...
uav_gain/GS_weight-uav_consume/uav_m-hive.lost/(hive_init.value)+(hive.xiufu)/(hive_init.value)+sensor_lost/10000;
%iobt支付函数
iobt_payoff_iobtbefore(strategy_hive_define,strategy_iot)=...
uav_consume/uav_m-uav_gain/GS_weight+hive.lost/(hive_init.value)+battle_ability/GS_weight+(sensor_add-sensor_lost)/10000-buildcost;
%%*********************************************************************%
%%所有相关参数
