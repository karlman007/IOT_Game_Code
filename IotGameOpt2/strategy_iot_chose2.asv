function [buildcost,battle_ability,hive,sensor_add,subnum,subwight,subair] =strategy_iot_chose2(strategy_iobt,subnum,subwight,subair,subairnum,hive,time_druing)
...
[on_numadd,on_valadd,subnum,subwight,subair] = strategy_on2(subnum,subwight,subair,on_m(1));
...
[on_numadd,on_valadd,subnum,subwight,subair] = strategy_on2(subnum,subwight,subair,ceil(5*rand(1))); 
sensor_add=on_numadd;%传感器增加的数量
%battle_ability=on_valadd;%战场权重增加量
buildcost=0.00005*sensor_add;%部署传感器花费的代价
hive.lost=0;%没有攻击敌方蜂巢
...
case 3%重新建立sn节点
sn_lose=find(cell2mat(subwight.SN)<=200);%判断每个区域内sn节点的数量
if ~isempty(sn_lose)
